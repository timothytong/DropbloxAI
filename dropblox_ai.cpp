...
int main(int argc, char** argv) {
    ...
    while (board.check(*board.block)) {
        struct block_info b_info = find_block_info(*board.block);
        cout << b_info.longest_side << ", " << b_info.rotation_times << endl;
...
/* This method returns the longest side of a block and how to rotate to get
 * to this rotation */
struct block_info find_block_info(const Block& block){
    struct block_info res = {0, 0, 0, 0};
    std::vector<vector<int> > offsets(7, vector<int>(7, 0));
    const int center = 3;
    int direction_furthest[4] = {0}; //For absolute length, order: UP DOWN LEFT RIGHT
    for(int i = 0; i < 10; i++) {
        // GREEDY
        Point p = block.offsets[i];
        // If number not in reasonable range, reject
        if(abs(p.i) > 3 || abs(p.j) > 3) continue;
        offsets[p.i + center][p.j + center] = 1;
        if(p.j > 0 && p.j > direction_furthest[0]) direction_furthest[0] = p.j; // UP
        else if(p.j < 0 && abs(p.j) > direction_furthest[1]) direction_furthest[1] = abs(p.j); // DOWN
        if(p.i < 0 && abs(p.i) > direction_furthest[2]) direction_furthest[2] = abs(p.i); // LEFT
        else if(p.i > 0 && p.i > direction_furthest[3]) direction_furthest[3] = p.i; // RIGHT
    }
    for(int i = 0; i < 7; i++){
        for(int j = 0; j < 7; j++){
            cout << offsets[i][j];
    ...
    res.absolute_length = direction_furthest[2] + direction_furthest[3];
    res.absolute_height = direction_furthest[0] + direction_furthest[1];

    bool left_done = false;
    bool up_done = false;
    int max_side = -1;
    int max_side_index = -1;
    // Now compute the longest side...
    int side_lengths[4]; // DOWN RIGHT UP LEFT (0, 1, 2, 3)
    for(int i = 0; i < 7; i++){
        // Scan row by row, col by col...
        int row_start = -1, row_end = -1;
        for(int j = 0; j < 3; j++){
            if(offsets[i][j] == 1 && row_start < 0) row_start = offsets[i][j];
            if(offsets[i][7 - j - 1] == 1 && row_end < 0) row_end = offsets[i][7 - j - 1];
        }
        cout << "ROW end - ROW start: " << row_end-row_start << endl;
        if (row_start > 0 && row_end > 0 && !left_done) {
            side_lengths[3] = (row_end - row_start) > 0 ? (row_end - row_start) : 1;
            left_done = true;
        }
        else if (row_start > 0 && row_end > 0) side_lengths[1] = (row_end - row_start) > 0 ? (row_end - row_start) : 1;
    }
    // same logic for up and down sides
    for(int i = 0; i < 7; i++){
        // Scan row by row, col by col...
        int col_start = -1, col_end = -1;
        for(int j = 0; j < 3; j++){
            if(offsets[j][i] == 1 && col_start < 0) col_start = offsets[j][i];
            if(offsets[j][7 - i - 1] == 1 && col_end < 0) col_end = offsets[j][7 - i - 1];
        }
        cout << "COL end - COL start: " << col_end- col_start << endl;
        if (col_start > 0 && col_end > 0 && !up_done) {
            side_lengths[2] = (col_end - col_start) > 0 ? (col_end - col_start) : 1;
            up_done = true;
        }
        else if (col_start > 0 && col_end > 0) side_lengths[1] = (col_end - col_start) > 0 ? (col_end - col_start) : 1;
    }

    for(int i = 0; i < 4; i++){
        if(side_lengths[i] > max_side){
            max_side = side_lengths[i];
            max_side_index = i;
        }
    }
    res.rotation_times = max_side_index;
    res.longest_side = max_side;
